...
- [x] Dashboard summary cards + revenue report — seller-scoped `GET /dashboard/summary` only; see drift below
  for what's deferred
- [x] Every empty, loading, and error state across the app — see status below for what two passes covered
- [x] Mobile responsiveness pass on the whole dashboard — see status below
- [x] Terms of service + privacy policy pages — placeholder copy, **not legally reviewed**; see drift below
- [ ] Backup verification: restore drill from PITR — tooling and a runbook shipped, and the drill itself
  now passes live in this environment; the checkbox stays open until it's run against real production
  infrastructure, see status below
- [x] Seller onboarding guide — in-app checklist + public `/panduan` page; see drift below
- [ ] **Pilot with 1–3 friendly sellers**

**Deliverable:** live product, first real transaction, first real withdrawal.

**Status: in progress, three passes — this one fully live-verified.** The first pass covered the dashboard
summary, six polish fixes, and the legal-placeholder pages. The second pass closed the remaining code-shaped
items — finishing the states/mobile audit at the route level, building the onboarding checklist and guide
page, and writing the backup/restore-drill tooling — but shipped typecheck/build-clean only, since Docker was
unavailable in that pass's environment. This third pass picked up once Docker Desktop was installed and
running: it created the missing `nagihin_test` database and applied all 13 migrations to it (`docker-compose`
only auto-creates `nagihin`, the dev database — `nagihin_test` is `test/integration-env.ts`'s separate target
and had never existed), then ran everything the second pass could only typecheck.

`pnpm run lint`, `pnpm run typecheck`, and `pnpm run build` are clean for both `apps/api` and `web`, including
the new `/panduan` route alongside the existing 28. The `apps/api` unit suite is green — 61 suites, 994 tests.
`test:integration` needed `--forceExit` in this environment — Jest completes every suite correctly but then
hangs waiting on open BullMQ/ioredis handles rather than exiting on its own; piping the command through `tail`
(as an earlier attempt in this pass did) hides this completely, since `tail` prints nothing until it sees EOF,
making a suite that finished successfully look identical to one that's still running. **After `--forceExit`,
the full suite is 13/13 passing, 85/85 tests** (up from an initial 12/13, 84/85 — the first two of the four
real bugs below, both in the new `reporting.int-spec.ts`, both fixed this pass). `scripts/backup/pg-backup.sh` and
`restore-drill.sh` both ran for real against the local Postgres: a dump, then a full restore into a throwaway
database with the assertions actually passing — 13/13 migrations, all 18 tables present with the expected row
counts, and 0 ledger reconciliation mismatches. A live smoke test booted both `api` and `web`, logged in as the
seeded seller over real HTTP, and confirmed `GET /health/ready` (Postgres/Redis/storage all `up`), `GET
/dashboard/summary` (real revenue/order/buyer numbers and all four onboarding flags computed correctly against
seeded data), the storefront SSR page for `@tokodemo` (real seeded products in the initial HTML), the
`/panduan` page and its footer link, an unauthenticated `/dashboard` redirect to `/masuk`, and a 404 on a
nonexistent product (the case the new empty-vs-error branch depends on).

New this pass, beyond the onboarding/states/backup work already described below: `apps/api/src/seed-dev.ts`
(wired as `pnpm --filter @nagihin/api run seed:dev`, or `pnpm prisma:seed:dev` from the root) — the
`seed/dev.ts` `.docs/06-database-roadmap.md §5` specs and no prior sprint built. It boots the real `AppModule`
via `NestFactory.createApplicationContext` and creates a demo seller/buyer/admin (`*@tokodemo.test` /
`password123`), a `@tokodemo` store, 6 products spanning all three types, and 11 orders across every reachable
`OrderStatus` by calling the real application services — `CheckoutService`, `MarkOrderPaidService`,
`LedgerService.creditHoldingForOrder`/`releaseToAvailableForOrder`, `ReleaseOrderService`, `CancelOrderService`,
`ExpireOrderService` — exactly the "generated by replaying the real domain commands, not inserted directly"
rule the seed strategy doc states, not a hand-rolled ledger fixture. `disputed`/`refunded` are the one
exception, inserted as bare status updates: no application service exists for either yet (Sprint 11).

Four real bugs surfaced only by actually running this pass's own new code against live infrastructure, not by
typecheck or lint:
1. `reporting.int-spec.ts`'s `createOrderRow` helper could insert a `status: 'released'` row with
   `released_at` still null, which migration `20260802000000_ordering`'s
   `orders_released_status_check` constraint rejects — fixed by setting `releasedAt` whenever the seeded
   status is `released`.
2. The same file's `createProductRow` built its slug from `id.slice(0, 8)` — the first 8 hex characters of a
   UUIDv7 are timestamp bits, not random ones, so two products created a few milliseconds apart in the same
   test collided on `(store_id, slug)`. The exact same class of bug Sprint 6 already hit and fixed for
   `OrderNumber` fixtures. Fixed by using the full id.
3. `reporting.int-spec.ts`'s `beforeEach` only cleaned up the tables its own fixtures touch, but
   `test:integration`'s database is shared across all thirteen suite files — a `store_buyers` row left behind
   by `crm.int-spec.ts` from an earlier run blocked this suite's indiscriminate `store.deleteMany()` on a
   `RESTRICT` foreign key. The exact same class of bug Sprint 7 already hit and fixed ("every existing
   `*.int-spec.ts` file needed withdrawal/bank_account cleanup added to its `beforeEach`"). Fixed by adopting
   the same full 18-table cleanup list every other suite in this codebase already uses.
4. Both backup scripts' `.env` loading (`set -a; source .env; set +a`) broke on the real `.env` in this
   environment: `JWT_PRIVATE_KEY`/`JWT_PUBLIC_KEY` are PEM blocks with unescaped spaces
   (`-----BEGIN PRIVATE KEY-----`), and bash's `source` word-splits an unquoted value on those spaces,
   producing `PRIVATE: command not found`. Fixed by grep-extracting only the one variable both scripts
   actually need (`DIRECT_DATABASE_URL`) instead of sourcing the whole file. Separately, `restore-drill.sh`'s
   Docker fallback path failed on Git Bash on Windows specifically: `docker.exe` is a native binary, and Bash's
   MSYS layer auto-converts POSIX-looking arguments like `/tmp/drill.dump` into Windows paths before `docker
   exec` ever sees them. Fixed with a `MSYS2_ARG_CONV_EXCL="/tmp"` prefix on the three commands that pass an
   in-container path — narrow enough that the surrounding `-f "$COMPOSE_FILE"` argument, which does need
   converting, is untouched. Neither of these two is Postgres/Docker-version-specific; both would hit any
   Windows + Git Bash + a PEM-shaped `.env` combination.

Also observed live, recorded but not fixed as out of scope for this sprint: `POST /auth/login`'s response
omits the caller's `store` summary (`UserResponseDto.fromDomain(authResult.user)` is called with no second
argument, unlike `GET /users/me`'s `UserResponseDto.fromDomain(user, await this.storeSummary(...))`) — a
pre-existing Sprint 2 gap, not a Sprint 8 regression. Harmless today because `web`'s `useCurrentUser()` always
calls `/users/me`, never reads the login response's `store` field, but worth a one-line fix whenever
`modules/identity` is touched next.

New files, complete list: `apps/api/src/seed-dev.ts`; `apps/api/src/modules/reporting/reporting.int-spec.ts`;
`DashboardSummaryService`/`DashboardSummaryResponseDto`'s `onboarding` block (`hasProfile`/
`hasPublishedProduct`/`hasBankAccount`/`hasFirstSale`), one more `$queryRaw` alongside the existing two;
`web/src/features/dashboard/components/onboarding-checklist.tsx` wired into `dashboard-summary-cards.tsx`;
`web/src/app/(dashboard)/dashboard/page.tsx` rewritten to branch on `useMyStore()` (storeless → `CreateStoreCard`,
matching `/dashboard/toko`'s existing pattern, instead of the summary query 4xx-ing behind `StoreOwnerGuard`);
`web/src/app/(marketing)/panduan/page.tsx` linked from `marketing-footer.tsx`; a shared `web/src/components/
data/route-error.tsx` plus `error.tsx` in every route group that lacked one (`(dashboard)/dashboard`,
`(admin)/admin`, `(buyer)/akun`, `(checkout)/checkout`, `(auth)`, `(marketing)`) and `web/src/app/
global-error.tsx`; `loading.tsx` for `(buyer)/akun`, `(checkout)/checkout/[orderNumber]`, and `(auth)`; a
404-vs-failure branch in the product-edit and buyer-order-detail pages via `ApiError.problem.status`;
`web/src/app/(buyer)/akun/layout.tsx` (new tab nav: Pesanan/Unduhan/Profil) and a real `buyer-profile-form.tsx`
replacing the "Belum tersedia" stub, backed by the existing `PATCH /users/me`; mobile fixes to
`keuangan/layout.tsx`'s tab nav and `bank-account-list.tsx`'s row (mirroring `social-links-editor.tsx`'s
existing treatment), plus 44px tap targets on the small icon buttons in both; `scripts/backup/pg-backup.sh`,
`restore-drill.sh`, and `assertions.sql` wired as `pnpm backup:dump`/`backup:drill`, documented in
`.docs/13-devops-testing-prod.md §3`.

Decisions and drift from the docs, recorded here rather than silently:
- **The onboarding flags ride on the existing `GET /dashboard/summary` response** rather than a new endpoint —
  one more `$queryRaw` in `Promise.all` alongside the two `DashboardSummaryService` already runs, so the
  checklist costs zero extra round trips. `hasProfile` checks `avatar_url IS NOT NULL AND bio IS NOT NULL` on
  `stores`; "store created" isn't a flag since `StoreOwnerGuard` reaching the controller already proves it.
- **The buyer profile page and a buyer account nav were closed as part of the states/mobile audit, not as
  their own line item.** `(buyer)/akun/profil` was a "Belum tersedia" dead end and `(buyer)/layout.tsx` had no
  navigation between `/akun`, `/akun/unduhan`, and `/akun/profil` — both are exactly the kind of gap "every
  empty, loading, and error state" was meant to catch, once the audit moved from component-level (already
  done pre-pass) to route-level.
- **Route-level `error.tsx` boundaries share one `RouteError` component** (`components/data/route-error.tsx`),
  re-exported by each group's `error.tsx` — Next.js requires the `'use client'` directive on the file Next
  itself resolves as the error boundary, not just on a component it re-exports, which the first `next build`
  of this pass caught immediately (`must be a Client Component`) since a bare `export { RouteError } from ...`
  doesn't propagate the directive.
- **The second pass's Docker gap is closed.** The onboarding flags, the new `reporting.int-spec.ts`, and both
  backup scripts have now all executed for real against a live Postgres/Redis, with results in the status
  paragraph above and the two int-spec bugs and two script bugs it surfaced fixed in this pass.
- **`scripts/backup/restore-drill.sh` supports both a local `psql`/`pg_restore`/`createdb`/`dropdb` toolchain
  and, when those aren't on `PATH`, the Docker Compose `postgres` service** (via `docker cp` + `exec`) — the
  same "works on the Windows dev box and the Linux VPS unchanged" precedent as `pg-backup.sh`'s `pg_dump`
  fallback. Its assertions (`assertions.sql`) check migration completeness, per-table row counts, and that
  `SUM(holding_delta)`/`SUM(available_delta)` per store from `balance_transactions` matches
  `stores.holding_balance`/`available_balance` — the same invariant `BalanceReconciler` and
  `ledger.int-spec.ts` assert, so a passing drill proves the restored data is coherent, not just that
  `pg_restore` exited `0`.
- **`GET /dashboard/summary` ships in a new `modules/reporting/` module, not `administration` or `ordering`.**
  `.docs/03-bounded-contexts.md §3.14` already names a read-only "Reporting" bounded context and assigns it
  exactly this deliverable — this is the first real slice of that context, mirroring
  `PlatformMetricsService`'s existing "raw `$queryRaw`, no repository indirection" pattern rather than
  inventing new architecture mid-sprint. It does not belong in `administration` (platform-wide/admin-role;
  this is seller-scoped) or `ordering` (would either duplicate ledger's balance math or reach across a
  contexts boundary).
- **"Revenue report" is `GET /dashboard/summary`'s today/7d/30d cards, not `.docs/05-api-roadmap.md §13`'s
  separate `GET /reports/revenue`** (arbitrary date range, daily/weekly/monthly grouping). That endpoint,
  along with `reports/profit`, `reports/products`, `reports/buyers`, and `reports/export`, stays deferred to
  Sprint 12/13 as the roadmap already states — none has a caller or UI yet. No new index was added for this;
  `orders`' existing `@@index([storeId, createdAt])` and `@@index([storeId, status, createdAt])` are enough at
  pilot scale (1–3 sellers). `@@index([storeId, paidAt])` is a flagged candidate for whenever the grouped
  endpoint ships.
- **"Today" is a UTC calendar day**, not store-local time — there is no per-store timezone concept anywhere in
  this codebase yet. A known simplification, not an oversight.
- **"Every empty, loading, and error state" and "mobile responsiveness pass" spanned two passes.** The first
  pass fixed six component-level gaps found by a targeted exploration: the dashboard home page itself
  (superseded by the summary cards above), missing `loading.tsx` on both `(admin)/admin` routes,
  `withdrawal-request-form.tsx`'s and `google-link-card.tsx`'s silent loading flashes (neither destructured
  `isPending` from its query), `social-links-editor.tsx`'s non-stacking mobile row, and `dashboard-topbar.tsx`'s
  under-44px avatar tap target. This second pass covers what was still route-level rather than
  component-level: an `error.tsx` per route group (six were missing one entirely, so a thrown error escaped to
  the root boundary and took the shell/sidebar with it) plus a `global-error.tsx` layout-crash backstop;
  `loading.tsx` for `(buyer)/akun`, `(checkout)/checkout/[orderNumber]`, and `(auth)`; a 404-vs-failure branch
  on the product-edit and buyer-order-detail pages, which previously showed one retryable error for both a
  deleted record and a network failure; the dead-end buyer profile stub and missing buyer nav; and the same
  mobile-stacking/tap-target treatment applied to `keuangan/layout.tsx`'s tabs and `bank-account-list.tsx`.
  Component-level states were otherwise already solid — `DataTable` gives most list pages all three states plus
  a mobile card fallback by construction. Deliberately left as-is: `features/products/components/product-list.tsx`'s
  card-only layout (no `DataTable`) — this is Sprint 4's own recorded decision, already mobile-first by
  construction, and `DataTable`'s sortable-header feature was itself cut in Sprint 6, so migrating wouldn't
  clearly win anything.
- **Terms of Service / Privacy Policy content is structural placeholder text**, not legal copy. Both pages
  (`/syarat-ketentuan`, `/kebijakan-privasi`) carry a visible on-page disclaimer, not just a code comment.
  This must go through a lawyer before this sprint's "pilot with 1–3 friendly sellers" bullet can be
  responsibly checked off — it addresses real fund custody/holding and personal data handling.
- **Sentry was surfaced during scoping and explicitly deselected** for this pass, not attempted or partially
  wired — still 100% absent (no package, no init, no `SENTRY_DSN`, no capture hook in the exception filters
  or worker bootstrap). Left unchecked rather than partially checked.
- **Not attempted, and not code work regardless**: production Midtrans account/live keys, the production
  checklist ([13 §3](./13-devops-testing-prod.md#3-production-checklist)), uptime monitoring + alert routing,
  actually running the restore drill against live infrastructure (the scripts exist now; running them there is
  the operator's), the storefront load test, and the pilot itself. All require the operator acting outside
  this tool — registering a real merchant account, restoring against production, or recruiting real sellers
  aren't things a coding session can do, independent of how much of Sprint 8 is otherwise done.
...
- [ ] Sentry, uptime monitoring, alert routing — not attempted this pass, deliberately deselected; see drift below
